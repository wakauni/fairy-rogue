...
const CARD_DATABASE = [
    ...
    // 7. マナシールド
    {
        ...
        effect: (user, target) => {
            const shieldVal = Math.floor(user.int * 2);
    ...
    // 8. ボディプレス
    {
        ...
        type: 'skill_custom',
        cost: 1,
        desc: 'DEFの値を攻撃力としてダメージを与える',
        effect: (user, target) => {
            // 簡易計算: DEF依存ダメージ
            let dmg = Math.max(1, user.def - Math.floor(target.def / 2));
            target.takeDamage(dmg);
            return { msg: `硬い体で体当たり！ ${dmg}のダメージ！` };
        }
    },
    // 9. ソウルキャノン
    {
    ...
    // 12. ピュージバースト
    {
        ...
        effect: (user, target) => {
            if (!user.currentStatus) return { msg: "解除する状態異常がない！（不発）" };
            
            user.clearAllStatus();
            let dmg = Math.max(1, Math.floor(user.int * 2.0) - Math.floor(target.int / 2));
            target.takeDamage(dmg);
            
            return { msg: `穢れを魔力に変換して撃ち出した！ ${dmg}のダメージ！` };
        }
    },
    // 13. リバウンドサイズ
    {
    ...
    // 8. 転禍為福
    {
        ...
        effect: (user, target) => {
            let rate = 0.2;
            if (user.currentStatus) rate = 0.4;
    ...
    // 14. 守りの光
    {
        ...
        effect: (user, target) => {
            ...
            const barrier = Math.floor(user.int * 1.0);
            user.heal(heal);
    ...
    // (合成結果) ミラクルライト
    {
        ...
        desc: 'HP全快、縮小解除、状態異常治癒、敵に特大ダメージ',
        cost: 0,
        effect: (user, target) => {
            user.hp = user.maxHp;
            user.shrinkLevel = 0;
...
const ACCESSORY_EFFECTS = [
    ...
    },
    {
        id: 'pin_small_1',
        name: '小人の留め針(Lv1)',
        desc: '縮小Lv1で固定され戻らなくなるが、INTが1.5倍になる',
        type: 'shrink_lock',
        minLevel: 1,
        stats: { int: 1.5 }
    },
    {
        id: 'pin_small_2',
        name: '小人の留め針(Lv2)',
        desc: '縮小Lv2で固定され戻らなくなるが、INTが2.0倍になる',
        type: 'shrink_lock',
        minLevel: 2,
        stats: { int: 2.0 }
    },
    {
        id: 'pin_small_3',
        name: '小人の留め針(Lv3)',
        desc: '縮小Lv3で固定され戻らなくなるが、INTが3.0倍になる',
        type: 'shrink_lock',
        minLevel: 3,
        stats: { int: 3.0 }
    },
...
const FAIRY_DIALOGUE_DATA = {
    ...
    idle_stripped_home: [
        // A. 恥じらい (Embarrassed)
        "うぅ、やっぱりスースーします……。誰も見てないとはいえ、落ち着きません！",
        "拠点でくらい服を着たいんですけど……『解放の証』が外れないんです。これ、呪われてませんか？",
        "手で隠しても隠しきれません……。あぁ、私、お嫁に行けないかも……。",
        ...
        // C. プレイヤーへの問いかけ (Teasing / Meta)
        "あの……私にこの格好をさせてるのって、趣味ですか？ ……えっち。",
        "そんなにまじまじと見ないでください。減るもんじゃないですけど……恥ずかしいものは恥ずかしいんです！",
        "防具を着ない「縛りプレイ」……ですよね？ 決して、やましい気持ちじゃないですよね？ 信じてますよ？",

        // D. ステータス実感 (Power)
        "頭が冴え渡って、体が軽いです！ ……見た目はともかく、今の私は最強かもしれません！",
        "魔力が遮られることなく溢れてきます。この姿なら、どんな難しい魔法も一瞬で組める気がします！",
        "防御力はゼロ、でも魔力は最大……！ 「やられる前にやる」、究極の戦闘スタイルですね！"
    ]
    ,

    // --- タッチ反応 (Touch Reactions) ---
    ...
    touch_stripped: {
        lv1: [
            ...
            "裸なんですから、少しは気を使って……！",
            "うぅ……隠す布がないと、防御できないです……。",
            "だ、ダメですって！ 見るだけ！ 見るだけにしてください！"
        ],
        lv2: [
            "ひゃうっ！？ ……ひ、皮膚が敏感になってるんですから！",
            "どこ触ってるんですか、もう……変態さん！",
            "んっ……！ くすぐったさが、普段の倍なんです……！",
            "風が当たるだけでもビリビリするのに……っ！"
        ],
        lv3: [
            "……そんなに、肌触りがいいですか？",
            "ふふっ、もう隠すのも面倒になっちゃいました。好きにしていいですよ？",
            "あなたの手、温かいですね……。なんだか、どうでもよくなってきちゃった。",
            "防御力ゼロの私を弄ぶなんて……ご主人様は悪い人ですね♪"
        ]
    },
    ...
    touch_shrink_1: {
        lv1: [
            "子供扱いしないでくださいね？",
            "つっつくと危ないですよ。転んじゃいます。",
            "むぅ……上から見下ろされるの、なんか癪です。",
            "ねえ、聞こえてますかー？"
        ],
        lv2: [
            "わわっ、指が大きくて怖いです……！",
            "押さないで！ バランスが取れないです～！",
            "うぅ、デコピンしないでくださいね？ 絶対ですよ？",
            "きゃっ！ ……もう、地震かと思いました。"
        ],
        lv3: [
            "小さいからって舐めないでくださいっ！ ガブッ！",
            "むぅ……いつか元のサイズに戻ったら、覚えててくださいね？",
            "そんなにイジワルするなら、鼻の頭に飛び乗っちゃいますよ！",
            "小さい私をいじめて楽しいですかーっ！ ぷんぷん！"
        ]
    },
    ...
    touch_shrink_2: {
        ...
        lv2: [
            "ひぃっ！ 揺らさないでぇ！",
            "圧が！ 指の圧が凄いです！ ぺしゃんこになっちゃう！",
            "きゃああ！ 捕まえないでぇ！ 食べないでぇ！",
            "わわわ、台風みたいに息を吹きかけないでくださいっ！"
        ],
        lv3: [
            "降参、降参です！ 弄ばないでくださいよぉ……。",
            "目が回るぅ……お星様が見えます……。",
            "もう抵抗しませんから、優しくしてください……。",
            "あぅぅ……おもちゃにしないでぇ……。"
        ]
    },
    ...
    touch_shrink_3: {
        ...
        lv2: [
            "わああっ！ 指のシワに挟まるっ！ 助けて！",
            "息だけで吹き飛びそうです……！ 風を起こさないでぇ！",
            "ぎゃー！ 指先が隕石みたいに降ってくるー！",
            "つままないで！ 粉々になっちゃいます！"
        ],
        lv3: [
            "もう虫けらの気分です……。煮るなり焼くなり好きにしてください。",
            "プチッてしないでくださいね……？ 約束ですよ……？",
            "どうせ私は豆粒です……。あなたの指紋に埋もれて暮らします……。",
            "……ふふ、巨大なご主人様に運命を握られるのも、悪くないかも？"
        ]
    }
};
